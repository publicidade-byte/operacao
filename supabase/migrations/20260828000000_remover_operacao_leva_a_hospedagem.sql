-- =====================================================================
-- REMOVER OPERAÇÃO LEVA A HOSPEDAGEM DAQUELA OPERAÇÃO
--
-- Agora que cada estadia aponta para a sua operação, tirar a operação da
-- solicitação tem de tirar as estadias dela junto: senão ficam linhas de
-- hotel para uma data que a solicitação não cobre mais — invisíveis no
-- painel e somando no custo.
--
-- O envelope da solicitação também passa a respeitar o que foi pedido
-- para cada operação, e não só o calendário dela.
-- =====================================================================
...
begin
  ...
  -- A estadia daquela operação sai com ela. Ficar seria guardar uma diária
  -- de uma data que a solicitação não cobre mais: some do painel, continua
  -- no custo.
  delete from hospedagem_detalhe h
   using colaboradores c
   where c.id = h.colaborador_id
     and c.solicitacao_id = p_solicitacao
     and h.edicao_id = p_edicao;

  -- Novo envelope e nova principal, a partir do que sobrou. Vale o que foi
  -- pedido para cada operação; sem pedido, o calendário dela.
  select min(coalesce(se.data_entrada, e.data_inicio)),
         max(coalesce(se.data_saida, e.data_fim))
  ...
  -- estadia antiga: recortar produz saída antes da entrada.
